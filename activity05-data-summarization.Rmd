...
**Response**:
It allows for other views of the data...
...
**Response**:
The distribution of the curve will be right skewed with the many of the observations holding smaller salary values
...
**Response**:
All of the median earnings are fairly close in value (between 30-60K) so you may actually see close to a normal distribution, maybe with a slight right skew. The means to do vary much from the median earnings. 
...
**Response**:
It changes the output from raw text to a prettier table display.
...
ggplot(data = college_recent_grads, mapping = aes(x=median, y=major_category)) +
         geom_boxplot() + geom_jitter(aes(color = major_category))
...
**Response**:
I made my description about median earnings overall by mistake, not about the distributions for individual major_categories so it's not relevant to this chart. It's interesting the outliers for engineering though. 
