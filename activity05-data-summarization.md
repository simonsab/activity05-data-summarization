...
**Response**: It allows for other views of the data…
...
college_recent_grads %>%
  summarise(
    observations = n(),
    mean_earnings = mean(median),
    stdev = sd(median),
    minimum_earnings = min(median),
    median_earnings = median(median),
    maximum_earnings = max(median)
  )
```

    ## # A tibble: 1 x 6
    ##   observations mean_earnings  stdev minimum_earnings median_earnings
    ##          <int>         <dbl>  <dbl>            <dbl>           <dbl>
    ## 1          173        40151. 11470.            22000           36000
    ## # … with 1 more variable: maximum_earnings <dbl>
...
**Response**: The distribution of the curve will be right skewed with
the many of the observations holding smaller salary values
...
| major\_category                     | observations | mean\_earnings |     stdev | minimum\_earnings | median\_earnings | maximum\_earnings |
|:------------------------------------|-------------:|---------------:|----------:|------------------:|-----------------:|------------------:|
...
**Response**: All of the median earnings are fairly close in value
(between 30-60K) so you may actually see close to a normal distribution,
maybe with a slight right skew. The means to do vary much from the
median earnings.
